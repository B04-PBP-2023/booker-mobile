...
import '../models/book_borrow.dart';
import '../models/book_bought.dart';

class BookDataCard extends StatefulWidget {
  BookDataCard({super.key, required this.idBuku, required this.reviewCount});
  final int idBuku;
...
class _BookDataCardState extends State<BookDataCard> {
  Future<List<Book>> fetchBook(CookieRequest request) async {
    var response = await request.get('/api/books');
    var bookBorrow = await request.get('/reviewbuku/get-borrow-json/');
    var bookBought = await request.get('/reviewbuku/get-bought-json/');
                              ...
                              onPressed: request.loggedIn
                                  ? () {
                                      showDialog(
                                          context: context,
                                          builder: (context) =>
                                              ReviewFormPage(idBuku: widget.idBuku));
                                    }
                                  : () {
                                      Navigator.push(
                                          context,
                      ...
                      children: [
                        Padding(
                            ...
                            child: ElevatedButton(
                              ...
                              child: const Text(
                                  "Pinjam atau beli buku ini terlebih dahulu untuk me-review",
